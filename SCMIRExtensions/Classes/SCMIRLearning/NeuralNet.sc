...
//			[input, weightsh[j], input *weightsh[j], biash[j]].postln;
	...
	*/
	trainExt {|trainingset, errortarget=0.05, maxepochs=100, action|
		...
		//clean up
		pipe.close;
		("rm " ++ cmdFileName).systemCmd; //was unixCmd, but interferes with itself asynchronously when multiple NNs trained in a row
